...
export async function POST(request: Request) { 
 try {
  ...
  cookies().set("token", login.token, {secure:true,httpOnly: true, sameSite: "strict", maxAge:60 * 30})
  cookies().set("role", login.user.role, {httpOnly: true, sameSite: "strict",  maxAge:60 * 30})
  cookies().set("id", login.user.id, {httpOnly: true, sameSite: "strict",  maxAge:60 * 30})
